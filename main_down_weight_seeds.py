...
def predict_bracket(matchups, team_mapping_df, reg_season, conference_df, final_model, region, features_kept):
    ...
    while len(matchups) > 1:
        ...
        for i in range(mid):
            ...
            curr_matchup = curr_matchup[features_kept]

            team_1_pred = final_model.predict(curr_matchup.to_numpy())[0]
            
            (winner_left, seed_left) = (left[0], left[2]) if team_1_pred > (0.5 + ERROR_CORRECTION) else (left[1], left[3])
            ...
            team_1_pred = final_model.predict(curr_matchup.to_numpy())[0]
            (winner_right, seed_right) = (right[0], right[2]) if team_1_pred > (0.5 + ERROR_CORRECTION) else (right[1], right[3])
...
def main():
    ...
    if not os.path.exists('models/regression_model_down.joblib'):
        ...
        print(f'number of features before removal of correlated features: {X.shape[1]}')
    ...
    if run_hist:
        ...
        for iteration, row in curr_tourney.iterrows():
            ...
            if predict > 0.5 + ERROR_CORRECTION:
                team_1 = 1
            else:
                team_1 = 0
    ...
    south_winner = predict_bracket(south_region, team_mapping_df, curr_season, conference_df, final_model, 'south',features_kept)
    ...
    east_winner = predict_bracket(east_region, team_mapping_df, curr_season, conference_df, final_model, 'east',features_kept)
    west_region = [
        # West Region (Florida No. 1)
        ('florida', 'norfolk state', 1, 16),
        ('uconn', 'oklahoma', 8, 9),
    ...
    west_winner = predict_bracket(west_region, team_mapping_df, curr_season, conference_df, final_model, 'west',features_kept)
    ...
    midwest_winner = predict_bracket(midwest_region, team_mapping_df, curr_season, conference_df, final_model, 'midwest',features_kept)
    ...
    overall_winner = predict_bracket(final_four_matchup, team_mapping_df, curr_season, conference_df, final_model, 'Final_four',features_kept)
